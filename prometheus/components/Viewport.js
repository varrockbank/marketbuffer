  ...
  template: `
    <div class="viewport">
      <div class="viewport-content">
        <h1 class="viewport-title">{{ currentContent.title }}</h1>
        <p class="viewport-text">{{ currentContent.text }}</p>
      </div>
    </div>
  `,
  setup() {
    const currentContent = Vue.computed(() => content[store.activeMenuItem] || { title: 'Unknown', text: '' });
